...
subrectangulos = [
    ...
    (7, 3),
    (8, 3),
]  # Población 4000 - Generaciones 50
...
def inicializar_poblacion(tamano, subrectangulos, contenedor_ancho, contenedor_alto):
    return [
        generar_individuo(subrectangulos, contenedor_ancho, contenedor_alto)
...
def calcular_fitness(individuo, contenedor_alto):
    ...
    altura_maxima = altura_maxima_individuo(
        individuo
    )  # altura del subrectángulo más alto
    penalizacion = 0
    aptitud = 0
    acum_superficie = 0
...
def seleccion(poblacion, contenedor_alto):
    ...
    for _ in range(len(poblacion)):
        ...
        if calcular_fitness(poblacion[i], contenedor_alto) > calcular_fitness(
            poblacion[j], contenedor_alto
        ):
...
def algoritmo_genetico(subrectangulos, contenedor_ancho, contenedor_alto, generaciones):
    poblacion = inicializar_poblacion(
        poblacion_tamano, subrectangulos, contenedor_ancho, contenedor_alto
    )
    ...
    for generacion in range(generaciones):
        ...
        print(
            f"Generación {generacion + 1}: {mejor_individuo_actual} Mejor Fitness = {mejor_fitness_actual}"
        )
    ...
    print(
        f"Mejor Solución encontrada: {mejor_individuo} con aptitud {mejor_fitness_global}"
    )
...
def graficar_resultado(
    mejor_individuo, historial_fitness, contenedor_ancho, contenedor_alto
):
    ...
    for rect in mejor_individuo:
        ...
        ax1.add_patch(
            plt.Rectangle(
                (x, y), ancho, alto, edgecolor="blue", facecolor="blue", alpha=0.5
            )
        )
...
if __name__ == "__main__":
    mejor_individuo, mejor_fitness, historial_fitness = algoritmo_genetico(
        subrectangulos, contenedor_ancho, contenedor_alto, generaciones
    )
    graficar_resultado(
        mejor_individuo, historial_fitness, contenedor_ancho, contenedor_alto
    )
